...
DISPATCH_CONFIRMATION_COUNTDOWN_SECONDS = 15
...
def _has_meaningful_conversation_context(
    *,
    ...
    if any((answer.answer or "").strip() for answer in patient_answers):
        return True
    if interaction is not None and (interaction.message_text or "").strip():
        return True
    if interaction is not None and interaction.new_fact_keys:
        return True
    if communication_state is None:
        return False
    if communication_state.patient_responded or communication_state.bystander_present:
        return True
    if communication_state.conscious is not None or communication_state.breathing_normal is not None:
        return True
    if communication_state.flags:
        return True
    if (communication_state.latest_message or "").strip():
        return True
    return False


def _canonical_reasoning_answers(
    *,
    patient_answers: list[PatientAnswer],
    communication_state: CommunicationState | None,
) -> list[PatientAnswer]:
    """Build a bounded reasoning payload from the canonical communication state."""

    if communication_state is None:
        return list(patient_answers)

    canonical_answers: list[PatientAnswer] = [
        PatientAnswer(
            question_id="response_status",
            answer=(
                "patient_responded"
                if communication_state.patient_responded
                else "no_response"
            ),
        ),
        PatientAnswer(
            question_id="bystander_presence",
            answer=(
                "bystander_present"
                if communication_state.bystander_present
                else "no_bystander_present"
            ),
        ),
    ]

    if communication_state.conscious is True:
        canonical_answers.append(PatientAnswer(question_id="consciousness", answer="patient conscious"))
    elif communication_state.conscious is False:
        canonical_answers.append(PatientAnswer(question_id="consciousness", answer="patient unconscious"))

    if communication_state.breathing_normal is True:
        canonical_answers.append(PatientAnswer(question_id="breathing", answer="breathing normally"))
    elif communication_state.breathing_normal is False:
        canonical_answers.append(PatientAnswer(question_id="breathing", answer="not breathing normally"))

    for flag in communication_state.flags:
        canonical_answers.append(
            PatientAnswer(question_id=f"flag_{flag}", answer=flag.replace("_", " "))
        )

    latest_message = (communication_state.latest_message or "").strip()
    if latest_message:
        canonical_answers.append(
            PatientAnswer(question_id="latest_responder_message", answer=latest_message)
        )

    combined_answers = [*canonical_answers, *patient_answers]
    deduped_answers: list[PatientAnswer] = []
    seen_pairs: set[tuple[str, str]] = set()
    for answer in combined_answers:
        key = (answer.question_id, answer.answer.strip().lower())
...
async def run_reasoning_assessment(
    ...
    assessment = FallAssessment(
        ...
        action=ActionSummary(
            ...
            countdown_seconds=(
                DISPATCH_CONFIRMATION_COUNTDOWN_SECONDS
                if next_action == "dispatch_pending_confirmation"
                else None
            ),
        ),
        response_plan=clinical_assessment.response_plan,
